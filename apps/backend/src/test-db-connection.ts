...
async function testConnection() {
  ...
  try {
    ...
    if (url.hostname.startsWith('db.') && url.hostname.endsWith('.supabase.co')) {
      ...
      console.log(`\n💡 프로젝트 참조: ${projectRef}`);
      console.log('⚠️  직접 연결이 실패하는 경우, Connection Pooling URL을 사용해보세요:');
      console.log(`   postgresql://postgres.${projectRef}:비밀번호@aws-0-ap-northeast-2.pooler.supabase.com:6543/postgres`);
      console.log('   또는 Supabase 대시보드의 Connection string에서 "Session mode" 또는 "Transaction mode" 확인\n');
    }
  } catch (error) {
  ...
  try {
    ...
    if (tablesResult.rows.length === 0) {
      console.log('  ⚠️  테이블이 없습니다. Supabase에서 테이블을 생성해주세요.');
    } else {
    ...
    if (tablesResult.rows.some((row) => row.table_name === 'comments')) {
      const commentsCount = await client.query('SELECT COUNT(*) as count FROM comments');
      console.log(`💬 comments 테이블 레코드 수: ${commentsCount.rows[0].count}`);
    }
