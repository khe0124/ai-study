...
**백엔드 관련**: 카카오 로그인 사용자 정보 조회(`GET /v2/user/me`)는 **클라이언트가 넘긴 Bearer 토큰만**으로 호출합니다.  
백엔드에 REST API 키·Admin 키 등 **별도 Kakao 환경 변수는 필요하지 않습니다.** (프론트엔드 JavaScript 키만 있으면 됨)
...
### 문제 1-1: "Kakao SDK 로드 타임아웃" 에러

**원인**: `developers.kakao.com`에서 SDK 스크립트가 15초 안에 로드되지 않음 (네트워크 지연, 광고/트래킹 차단, 방화벽 등)

**해결**:
1. **광고 차단/프라이버시 확장 프로그램**에서 `developers.kakao.com` 허용 또는 시크릿 모드에서 테스트
2. **네트워크**: 다른 네트워크(Wi-Fi/모바일) 또는 VPN 해제 후 재시도
3. 브라우저 개발자 도구 → Network 탭에서 `kakao.js` 요청이 실패(빨간색)인지, pending이 오래 가는지 확인
4. 로그인 페이지 진입 시 SDK가 미리 로드되므로, 페이지 새로고침 후 잠시 기다렸다가 카카오 버튼 클릭
5. 코드상으로는 타임아웃 15초, 실패 시 스크립트 제거 후 1회 재시도가 적용되어 있음
